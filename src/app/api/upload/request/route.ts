...
// Canonical extension derived server-side from the validated MIME type.
// Never use the client-supplied filename extension — it can be forged.
...
export async function POST(req: NextRequest) {
  ...
  if ((!stageId && !questionId) || (stageId && questionId)) {
    return NextResponse.json(
      { error: "Provide either stageId or questionId, not both" },
      { status: 400 },
    );
  }
